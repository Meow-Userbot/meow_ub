...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    meow = await sticcers[c_lick].click("me", hide_via=True)
    if meow:
        await borg.send_file(int(chat_id), meow, reply_to=reply_to_id)
        await meow.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    meow = await sticcers[0].click("me", hide_via=True)
    if meow:
        await bot.send_file(int(chat_id), meow, reply_to=reply_to_id)
        await meow.delete()
