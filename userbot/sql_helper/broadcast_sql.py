...
class MeowBroadcast(BASE):
    __tablename__ = "Meowbroadcast"
    ...
    def __repr__(self):
        return "<Meow Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, MeowBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


MeowBroadcast.__table__.create(checkfirst=True)

MeowBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with MeowBROADCAST_INSERTION_LOCK:
        broadcast_group = MeowBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with MeowBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(MeowBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with MeowBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(MeowBroadcast.keywoard)
            .filter(MeowBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(MeowBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(MeowBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(MeowBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(MeowBroadcast).all()
        for x in all_groups:
