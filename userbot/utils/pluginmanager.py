...
from ..helpers.utils import _Meowtools, _Meowutils, _format, install_pip, reply_id
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod.Config = Config
        mod._format = _format
        mod.tgbot = Meowub.tgbot
        mod.sudo_cmd = sudo_cmd
        mod.CMD_HELP = CMD_HELP
        mod.reply_id = reply_id
        mod.admin_cmd = admin_cmd
        mod._Meowutils = _Meowutils
        mod._Meowtools = _Meowtools
...
def remove_plugin(shortname):
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(Meowub._event_builders))):
            ev, cb = Meowub._event_builders[i]
            if cb.__module__ == name:
                del Meowub._event_builders[i]
    except BaseException:
        raise ValueError
