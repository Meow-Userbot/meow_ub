...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await Meowub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await Meowub.upload_file(autophoto_path)
        try:
            await Meowub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await Meowub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await Meowub(
                    functions.photos.DeletePhotosRequest(
                        await Meowub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await Meowub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        Meow = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(Meow, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await Meowub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await Meowub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await Meowub(functions.photos.UploadProfilePhotoRequest(file))
        await _Meowutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
Meowub.loop.create_task(autopfp_start())
Meowub.loop.create_task(autopicloop())
Meowub.loop.create_task(digitalpicloop())
Meowub.loop.create_task(bloom_pfploop())
Meowub.loop.create_task(autoname_loop())
Meowub.loop.create_task(autobio_loop())
Meowub.loop.create_task(custompfploop())
